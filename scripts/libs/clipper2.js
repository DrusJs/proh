...
class Minkowski {
  static minkowskiInternal(pattern, path, isSum, isClosed) {
    ...
    for (const pathPt of path) {
      const path2 = [];
      if (isSum) {
        for (const basePt of pattern) path2.push({
          x: pathPt.x + basePt.x,
          y: pathPt.y + basePt.y
        });
      } else {
    ...
    for (let i = delta; i < pathLen; i++) {
      for (let j = 0; j < patLen; j++) {
        ...
        if (! Clipper.isPositive(quad)) result.push(Clipper.reversePath(quad)); else result.push(quad);
        h = j;
      }
      g = i;
    }
...
class ClipperOffset {
  ...
  execute(delta, solution) {
    ...
    if (this._groupList[0].pathsReversed) c.execute(ClipType.Union, FillRule.Negative, solution); else c.execute(ClipType.Union, FillRule.Positive, solution);
  }
  executePolytree(delta, polytree) {
    ...
    c.reverseSolution = this.ReverseSolution !== this._groupList[0].pathsReversed;
    c.addSubjectPaths(this._solution);
    if (this._groupList[0].pathsReversed) c.executePolyTree(ClipType.Union, FillRule.Negative, polytree); else c.executePolyTree(ClipType.Union, FillRule.Positive, polytree);
  }
  ...
  static getBoundsAndLowestPolyIdx(paths) {
    ...
    for (let i = 0; i < paths.length; i++) {
      for (const pt of paths[i]) {
        ...
        if (pt.x > rec.right) rec.right = pt.x; else if (pt.x < rec.left) rec.left = pt.x;
      }
    }
    return {
      index,
  ...
  offsetPoint(group, path, j, k) {
    ...
    if (sinA > 1.0) cosA = 1.0; else if (sinA < - 1.0) cosA = - 1.0;
    if (typeof  this.DeltaCallback !== "undefined") {
    ...
    } else if (cosA > 0.99 || this._joinType == JoinType.Bevel) this.doBevel(group, path, j, k); else if (this._joinType == JoinType.Round) this.doRound(group, path, j, k, Math.atan2(sinA, cosA)); else this.doSquare(group, path, j, k);
    k = j;
  }
...
class RectClip64 {
  ...
  static isClockwise(prev, curr, prevPt, currPt, rectMidPoint) {
    if (this.areOpposites(prev, curr)) return InternalClipper.crossProduct(prevPt, rectMidPoint, currPt) < 0; else return this.headingClockwise(prev, curr);
  }
  ...
  static getEdgesForPt(pt, rec) {
    let result = 0;
    if (pt.x === rec.left) result = 1; else if (pt.x === rec.right) result = 4;
    if (pt.y === rec.top) result += 2; else if (pt.y === rec.bottom) result += 8;
    return result;
  }
  ...
  addCorner(prev, curr) {
    if (RectClip64.headingClockwise(prev, curr)) this.add(this.rectPath[prev]); else this.add(this.rectPath[curr]);
  }
  ...
  static getLocation(rec, pt) {
    ...
    if (pt.x < rec.left) loc = Location.left; else if (pt.x > rec.right) loc = Location.right; else if (pt.y < rec.top) loc = Location.top; else if (pt.y > rec.bottom) loc = Location.bottom; else loc = Location.inside;
    return {
      success: true,
      loc
    };
  ...
  static getSegmentIntersection(p1, p2, p3, p4) {
    ...
    if (res1 === 0) {
      ...
      if (res2 === 0) return {
        ip,
        success: false
      }; else if (equals(p1, p3) || equals(p1, p4)) return {
        ip,
        success: true
      }; else if (RectClip64.isHorizontal(p3, p4)) return {
        ip,
        success: p1.x > p3.x === p1.x < p4.x
      }; else return {
        ip,
        success: p1.y > p3.y === p1.y < p4.y
      };
    } else if (res2 === 0) {
      ip = p2;
      if (equals(p2, p3) || equals(p2, p4)) return {
        ip,
        success: true
      }; else if (RectClip64.isHorizontal(p3, p4)) return {
        ip,
        success: p2.x > p3.x === p2.x < p4.x
      }; else return {
        ip,
        success: p2.y > p3.y === p2.y < p4.y
      };
    }
    ...
    if (res3 === 0) {
      ip = p3;
      if (equals(p3, p1) || equals(p3, p2)) return {
        ip,
        success: true
      }; else if (RectClip64.isHorizontal(p1, p2)) return {
        ip,
        success: p3.x > p1.x === p3.x < p2.x
      }; else return {
        ip,
        success: p3.y > p1.y === p3.y < p2.y
      };
    } else if (res4 === 0) {
      ip = p4;
      if (equals(p4, p1) || equals(p4, p2)) return {
        ip,
        success: true
      }; else if (RectClip64.isHorizontal(p1, p2)) return {
        ip,
        success: p4.x > p1.x === p4.x < p2.x
      }; else return {
        ip,
        success: p4.y > p1.y === p4.y < p2.y
      };
    }
    if (res3 > 0 === res4 > 0) return {
      ip: new Point64(0, 0),
      success: false
    };
  ...
  static getIntersection(rectPath, p, p2, loc) {
    ...
    switch (loc) {
      case Location.left:
        if ((result = RectClip64.getSegmentIntersection(p, p2, rectPath[0], rectPath[3])).success) return {
          success: true,
          loc,
          ip: result.ip
      ...
      case Location.right:
        if ((result = RectClip64.getSegmentIntersection(p, p2, rectPath[1], rectPath[2])).success) return {
          success: true,
          loc,
          ip: result.ip
        }; else if (p.y < rectPath[0].y && (result = RectClip64.getSegmentIntersection(p, p2, rectPath[0], rectPath[1])).success) {
      ...
      case Location.top:
        if ((result = RectClip64.getSegmentIntersection(p, p2, rectPath[0], rectPath[1])).success) return {
          success: true,
          loc,
          ip: result.ip
      ...
      case Location.bottom:
        if ((result = RectClip64.getSegmentIntersection(p, p2, rectPath[2], rectPath[3])).success) return {
          success: true,
          loc,
          ip: result.ip
  ...
  getNextLocation(path, context) {
    switch (context.loc) {
      case Location.left:
        ...
        if (path[context.i].x >= this.rect.right) context.loc = Location.right; else if (path[context.i].y <= this.rect.top) context.loc = Location.top; else if (path[context.i].y >= this.rect.bottom) context.loc = Location.bottom; else context.loc = Location.inside;
        break;
      case Location.top:
        while (context.i <= context.highI && path[context.i].y <= this.rect.top) context.i++;
        if (context.i > context.highI) break;
        if (path[context.i].y >= this.rect.bottom) context.loc = Location.bottom; else if (path[context.i].x <= this.rect.left) context.loc = Location.left; else if (path[context.i].x >= this.rect.right) context.loc = Location.right; else context.loc = Location.inside;
        break;
      case Location.right:
        while (context.i <= context.highI && path[context.i].x >= this.rect.right) context.i++;
        if (context.i > context.highI) break;
        if (path[context.i].x <= this.rect.left) context.loc = Location.left; else if (path[context.i].y <= this.rect.top) context.loc = Location.top; else if (path[context.i].y >= this.rect.bottom) context.loc = Location.bottom; else context.loc = Location.inside;
        break;
      case Location.bottom:
        while (context.i <= context.highI && path[context.i].y >= this.rect.bottom) context.i++;
        if (context.i > context.highI) break;
        if (path[context.i].y <= this.rect.top) context.loc = Location.top; else if (path[context.i].x <= this.rect.left) context.loc = Location.left; else if (path[context.i].x >= this.rect.right) context.loc = Location.right; else context.loc = Location.inside;
        break;
      case Location.inside:
        while (context.i <= context.highI) {
          if (path[context.i].x < this.rect.left) context.loc = Location.left; else if (path[context.i].x > this.rect.right) context.loc = Location.right; else if (path[context.i].y > this.rect.bottom) context.loc = Location.bottom; else if (path[context.i].y < this.rect.top) context.loc = Location.top; else {
  ...
  executeInternal(path) {
    ...
    while (i <= highI) {
      ...
      if (loc == Location.inside) {
        ...
        const ip2 = result.ip;
        if (prevCrossLoc != Location.inside && prevCrossLoc != loc) this.addCorner(prevCrossLoc, loc);
        if (firstCross == Location.inside) {
          firstCross = loc;
          startLocs.push(prev);
        }
        ...
        loc = crossingLoc;
        if (firstCross == Location.inside) firstCross = crossingLoc;
      }
      this.add(ip);
    }
    if (firstCross == Location.inside) {
      ...
      if (loc != firstCross) this.addCornerByRef(loc, RectClip64.headingClockwise(loc, firstCross));
    }
  }
  execute(paths) {
    const result = [];
    ...
    for (const path of paths) {
      ...
      if (! this.rect.intersects(this.pathBounds)) continue; else if (this.rect.containsRect(this.pathBounds)) {
        result.push(path);
        continue;
      }
        ...
        if (edgeSet2 !== 0 && op2.edge === undefined) {
          ...
          for (let j = 0; j < 4; ++j) {
            if ((combinedSet & 1 << j) !== 0) {
              if (RectClip64.isHeadingClockwise(op2.prev.pt, op2.pt, j)) RectClip64.addToEdge(this.edges[j * 2], op2); else RectClip64.addToEdge(this.edges[j * 2 + 1], op2);
            }
          }
        }
        edgeSet1 = edgeSet2;
  ...
  tidyEdgePair(idx, cw, ccw) {
    ...
    while (i < cw.length) {
      ...
      if (op.next === op.prev || op.pt === op.prev.pt) {
        ...
        if (opIsLarger === cwIsTowardLarger) cw[i] = op; else ccw[j] = op;
        if (op2IsLarger === cwIsTowardLarger) cw[i] = op2; else ccw[j] = op2;
      }
    }
  }
  getPath(op) {
...
class Clipper {
  ...
  static makePath(arr) {
    ...
    const p = new Path64();
    for (let i = 0; i < len; i++) p.push(new Point64(arr[i * 2], arr[i * 2 + 1]));
    return p;
  }
  ...
  static getPrior(current, high, flags) {
    if (current === 0) current = high; else current--;
...
class ClipperBase {
  ...
  static getPrevHotEdge(ae) {
    ...
    while (prev && (ClipperBase.isOpen(prev) || ! ClipperBase.isHotEdgeActive(prev))) prev = prev.prevInAEL;
    return prev;
  }
  static isFront(ae) {
  ...
  static getCurrYMaximaVertex_Open(ae) {
    ...
    if (ae.windDx > 0) {
      while (result.next.pt.y === result.pt.y && (result.flags & (VertexFlags.OpenEnd | VertexFlags.LocalMax)) === VertexFlags.None) result = result.next;
    } else {
      while (result.prev.pt.y === result.pt.y && (result.flags & (VertexFlags.OpenEnd | VertexFlags.LocalMax)) === VertexFlags.None) result = result.prev;
    }
  ...
  static swapOutrecs(ae1, ae2) {
    ...
    if (or1) {
      if (ae1 === or1.frontEdge) or1.frontEdge = ae2; else or1.backEdge = ae2;
    }
    if (or2) {
      if (ae2 === or2.frontEdge) or2.frontEdge = ae1; else or2.backEdge = ae1;
    }
    ae1.outrec = or2;
    ae2.outrec = or1;
  }
    ...
    if (ae2 === undefined) {
      ...
      if (ae2.windCount * ae2.windDx < 0) {
        if (Math.abs(ae2.windCount) > 1) {
          if (ae2.windDx * ae.windDx < 0) ae.windCount = ae2.windCount; else ae.windCount = ae2.windCount + ae.windDx;
        } else {
  ...
  setWindCountForOpenPathEdge(ae) {
    ...
    if (this._fillrule === FillRule.EvenOdd) {
      let cnt1 = 0, cnt2 = 0;
      while (ae2 !== ae) {
        if (ClipperBase.getPolyType(ae2) === PathType.Clip) cnt2++; else if (! ClipperBase.isOpen(ae2)) cnt1++;
        ae2 = ae2.nextInAEL;
      }
      ...
      while (ae2 !== ae) {
        if (ClipperBase.getPolyType(ae2) === PathType.Clip) ae.windCount2 += ae2.windDx; else if (! ClipperBase.isOpen(ae2)) ae.windCount += ae2.windDx;
        ae2 = ae2.nextInAEL;
      }
    }
  }
  ...
  addLocalMinPoly(ae1, ae2, pt, isNew = false) {
    ...
    if (ClipperBase.isOpen(ae1)) {
      ...
      outrec.isOpen = true;
      if (ae1.windDx > 0) ClipperBase.setSides(outrec, ae1, ae2); else ClipperBase.setSides(outrec, ae2, ae1);
    } else {
      outrec.isOpen = false;
      ...
      if (prevHotEdge) {
        ...
        if (ClipperBase.outrecIsAscending(prevHotEdge) === isNew) ClipperBase.setSides(outrec, ae2, ae1); else ClipperBase.setSides(outrec, ae1, ae2);
      } else {
        outrec.owner = undefined;
        if (isNew) ClipperBase.setSides(outrec, ae1, ae2); else ClipperBase.setSides(outrec, ae2, ae1);
      }
    }
  ...
  addLocalMaxPoly(ae1, ae2, pt) {
    ...
    if (ClipperBase.isFront(ae1) === ClipperBase.isFront(ae2)) {
      if (ClipperBase.isOpenEndActive(ae1)) ClipperBase.swapFrontBackSides(ae1.outrec); else if (ClipperBase.isOpenEndActive(ae2)) ClipperBase.swapFrontBackSides(ae2.outrec); else {
        this._succeeded = false;
        return undefined;
      }
    }
    ...
    if (ae1.outrec === ae2.outrec) {
      ...
      if (this._using_polytree) {
        ...
        if (e === undefined) outrec.owner = undefined; else ClipperBase.setOwner(outrec, e.outrec);
      }
      ClipperBase.uncoupleOutRec(ae1);
    } else if (ClipperBase.isOpen(ae1)) {
      if (ae1.windDx < 0) ClipperBase.joinOutrecPaths(ae1, ae2); else ClipperBase.joinOutrecPaths(ae2, ae1);
    } else if (ae1.outrec.idx < ae2.outrec.idx) ClipperBase.joinOutrecPaths(ae1, ae2); else ClipperBase.joinOutrecPaths(ae2, ae1);
    return result;
  }
  ...
  static addOutPt(ae, pt) {
    ...
    if (toFront && pt == opFront.pt) return opFront; else if (! toFront && pt == opBack.pt) return opBack;
  ...
  static findEdgeWithMatchingLocMin(e) {
    ...
    while (result) {
      ...
      if (! ClipperBase.isHorizontal(result) && e.bot !== result.bot) result = undefined; else result = result.nextInAEL;
    }
    result = e.prevInAEL;
    while (result) {
      if (result.localMin === e.localMin) return result;
  ...
  intersectEdges(ae1, ae2, pt) {
    ...
    if (ae1.localMin.polytype === ae2.localMin.polytype) {
      if (this._fillrule === FillRule.EvenOdd) {
        ...
        if (ae1.windCount + ae2.windDx === 0) ae1.windCount = - ae1.windCount; else ae1.windCount += ae2.windDx;
        if (ae2.windCount - ae1.windDx === 0) ae2.windCount = - ae2.windCount; else ae2.windCount -= ae1.windDx;
      }
    } else {
      if (this._fillrule !== FillRule.EvenOdd) ae1.windCount2 += ae2.windDx; else ae1.windCount2 = ae1.windCount2 === 0 ? 1 : 0;
      if (this._fillrule !== FillRule.EvenOdd) ae2.windCount2 -= ae1.windDx; else ae2.windCount2 = ae2.windCount2 === 0 ? 1 : 0;
    }
    switch (this._fillrule) {
      case FillRule.Positive:
        oldE1WindCount = ae1.windCount;
  ...
  deleteFromAEL(ae) {
    ...
    if (prev) prev.nextInAEL = next; else this._actives = next;
  ...
  adjustCurrXAndCopyToSEL(topY) {
    ...
    while (ae) {
      ...
      if (ae.joinWith === JoinWith.Left) ae.curX = ae.prevInAEL.curX; else ae.curX = ClipperBase.topX(ae, topY);
      ae = ae.nextInAEL;
    }
  }
        ...
        while (left !== lEnd && right !== rEnd) {
          if (right.curX < left.curX) {
            ...
            if (left === currBase) {
              currBase = tmp;
              currBase.jump = rEnd;
              if (prevBase === undefined) this._sel = currBase; else prevBase.jump = currBase;
            }
          } else {
            left = left.nextInSEL;
          }
  ...
  doHorizontal(horz) {
    ...
    if (vertex_max && ! horzIsOpen && vertex_max !== horz.vertexTop) ClipperBase.trimHorz(horz, this.preserveCollinear);
    ...
    for (; ; ) {
      ...
      while (ae) {
        if (ae.vertexTop === vertex_max) {
          ...
          if (ClipperBase.isHotEdgeActive(horz)) {
            ...
            if (isLeftToRight) this.addLocalMaxPoly(horz, ae, horz.top); else this.addLocalMaxPoly(ae, horz, horz.top);
          }
      ...
      if (horzIsOpen && ClipperBase.isOpenEndActive(horz)) {
        if (ClipperBase.isHotEdgeActive(horz)) {
          ClipperBase.addOutPt(horz, horz.top);
          if (ClipperBase.isFront(horz)) horz.outrec.frontEdge = undefined; else horz.outrec.backEdge = undefined;
          horz.outrec = undefined;
        }
        this.deleteFromAEL(horz);
        return;
      } else if (ClipperBase.nextVertex(horz).pt.y !== horz.top.y) break;
      if (ClipperBase.isHotEdgeActive(horz)) {
        ClipperBase.addOutPt(horz, horz.top);
      }
      this.updateEdgeIntoAEL(horz);
    ...
    if (ClipperBase.isOpenEndActive(ae)) {
      ...
      if (! ClipperBase.isHorizontal(ae)) {
        if (ClipperBase.isHotEdgeActive(ae)) {
          if (ClipperBase.isFront(ae)) ae.outrec.frontEdge = undefined; else ae.outrec.backEdge = undefined;
          ae.outrec = undefined;
        }
        this.deleteFromAEL(ae);
      }
  ...
  checkJoinLeft(e, pt, checkCurrX = false) {
    ...
    if (! prev || ClipperBase.isOpen(e) || ClipperBase.isOpen(prev) || ! ClipperBase.isHotEdgeActive(e) || ! ClipperBase.isHotEdgeActive(prev)) return;
    ...
    if (e.outrec.idx === prev.outrec.idx) this.addLocalMaxPoly(prev, e, pt); else if (e.outrec.idx < prev.outrec.idx) ClipperBase.joinOutrecPaths(e, prev); else ClipperBase.joinOutrecPaths(prev, e);
    prev.joinWith = JoinWith.Right;
    e.joinWith = JoinWith.Left;
  }
  checkJoinRight(e, pt, checkCurrX = false) {
    const next = e.nextInAEL;
    if (ClipperBase.isOpen(e) || ! ClipperBase.isHotEdgeActive(e) || ClipperBase.isJoined(e) || ! next || ClipperBase.isOpen(next) || ! ClipperBase.isHotEdgeActive(next)) return;
    ...
    if (e.outrec.idx === next.outrec.idx) this.addLocalMaxPoly(e, next, pt); else if (e.outrec.idx < next.outrec.idx) ClipperBase.joinOutrecPaths(e, next); else ClipperBase.joinOutrecPaths(next, e);
  ...
  static updateHorzSegment(hs) {
    ...
    if (result) hs.leftOp.horz = hs; else hs.rightOp = undefined;
    return result;
  }
  ...
  convertHorzSegsToJoins() {
    ...
    this._horzSegList.sort((hs1, hs2) => {
      ...
      } else if (! hs2.rightOp) return - 1; else return hs1.leftOp.pt.x - hs2.leftOp.pt.x;
    });
    for (let i = 0; i < k - 1; i++) {
  ...
  static pointInOpPolygon(pt, op) {
    ...
    op2 = op.next;
    while (op2 !== op) {
      if (isAbove) while (op2 !== op && op2.pt.y < pt.y) op2 = op2.next; else while (op2 !== op && op2.pt.y > pt.y) op2 = op2.next;
      if (op2 === op) break;
      if (op2.pt.y === pt.y) {
        if (op2.pt.x === pt.x || op2.pt.y === op2.prev.pt.y && pt.x < op2.prev.pt.x !== pt.x < op2.pt.x) return PointInPolygonResult.IsOn;
        op2 = op2.next;
        if (op2 === op) break;
        continue;
      }
      if (op2.pt.x <= pt.x || op2.prev.pt.x <= pt.x) {
        if (op2.prev.pt.x < pt.x && op2.pt.x < pt.x) val = 1 - val; else {
    ...
    if (val === 0) return PointInPolygonResult.IsOutside; else return PointInPolygonResult.IsInside;
  }
  static path1InsidePath2(op1, op2) {
    ...
    do {
      ...
      if (result === PointInPolygonResult.IsOutside) ++outside_cnt; else if (result === PointInPolygonResult.IsInside) --outside_cnt;
      op = op.next;
  ...
  static buildPath(op, reverse, isOpen, path) {
    ...
    if (path.length === 3 && this.isVerySmallTriangle(op2)) return false; else return true;
  }
  ...
  checkBounds(outrec) {
    ...
    this.cleanCollinear(outrec);
    if (outrec.pts === undefined || ! ClipperBase.buildPath(outrec.pts, this.reverseSolution, false, outrec.path)) return false;
  ...
  recursiveCheckOwners(outrec, polypath) {
    ...
    while (outrec.owner !== undefined) {
      if (outrec.owner.splits !== undefined && this.checkSplitOwner(outrec, outrec.owner.splits)) break; else if (outrec.owner.pts !== undefined && this.checkBounds(outrec.owner) && ClipperBase.path1InsidePath2(outrec.pts, outrec.owner.pts)) break;
  ...
  buildTree(polytree, solutionOpen) {
    ...
    while (i < this._outrecList.length) {
      ...
      if (outrec.isOpen) {
        ...
        if (ClipperBase.buildPath(outrec.pts, this.reverseSolution, true, open_path)) solutionOpen.push(open_path);
        continue;
      }
...
class PolyPathBase {
  ...
  toStringInternal(idx, level) {
    ...
    for (let i = 0; i < this.children.length; i++) if (this.children[i].children.length > 0) result += this.children[i].toStringInternal(i, level + 1);
    return result;
  }
  toString() {
    ...
    for (let i = 0; i < this.children.length; i++) if (this.children[i].children.length > 0) result += this.children[i].toStringInternal(i, 1);
    return result + '\n';
  }
}
...
function midPointRound(value, mode = MidpointRounding.ToEven) {
...
class InternalClipper {
  ...
  static getIntersectPoint(ln1a, ln1b, ln2a, ln2b) {
    ...
    if (t <= 0.0) ip = ln1a; else if (t >= 1.0) ip = ln1b; else ip = new Point64(Math.trunc(ln1a.x + t * dx1), Math.trunc(ln1a.y + t * dy1));
    return {
      ip,
      success: true
    };
  ...
  static pointInPolygon(pt, polygon) {
    ...
    if (isAbove !== startingAbove) {
      if (i === len) i = 0; else d = InternalClipper.crossProduct(polygon[i - 1], polygon[i], pt);
      if (d === 0) return PointInPolygonResult.IsOn;
      if (d < 0 === isAbove) val = 1 - val;
    }
